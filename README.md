# Frank's Life List

## 中文

Frank's Life List 是一个私密、可分享的 life target list，用来记录你想完成、想经历、想购买、想实现的事情。

它是 Frank 系列作品的一部分：一个轻量的 web app，无需注册，数据保留在本地设备里，并且让分享列表这件事变得很简单。你可以把它当作人生清单、to-do list、grocery list，或者和朋友一起使用的共享 checklist。

### 为什么做它

很多列表应用都会要求你先注册账号、把数据上传到云端，并且把所有 checklist 都做成同一种产品逻辑。

- 无需注册
- 不强制云端存储
- 浏览器本地加密保存
- 压缩后的轻量分享与同步
- 可与朋友异步对齐、对比进度
- 可用于人生目标、日常任务、旅行计划、购物清单等场景

### 核心特性

- **默认更私密**：你的列表数据保存在浏览器本地，而不是依赖账号系统托管。
- **无需注册即可开始**：打开应用就能直接使用。
- **本地加密 + 压缩存储**：状态数据在浏览器中加密，并经过压缩，便于轻量分享。
- **异步同步流程**：你可以把某一组 list 分享给朋友，在不依赖实时后端的情况下进行对齐和对比。
- **简单分享**：支持分享链接和二维码，便于轻量协作。
- **用途灵活**：既适合做 life list，也适合 bucket list、to-do list、checklist 或 grocery list。

### 适合谁

- 想用一个简洁的 web app 来记录人生目标和个人清单的人
- 想要一个不需要注册账号就能分享的 checklist 的人
- 想找一个比云端优先 to-do app 更轻量替代方案的人
- 想用更注重隐私的方式保存和对比列表的人

### 快速开始

1. 安装依赖：

```bash
npm install
```

2. 启动开发环境：

```bash
npm run dev
```

3. 构建生产版本：

```bash
npm run build
```

### 技术栈

- React
- Vite
- TypeScript
- Tailwind CSS
- Zustand
- LZ-String
- Web Crypto API
- Lucide React
- Motion

## English

Frank's Life List is a private, shareable life target list for capturing the things you want to do, finish, buy, or experience.

It is designed as part of the Frank's series: a lightweight web app that works without signup, keeps data on-device, and makes list sharing feel simple. You can use it as a life goals list, a to-do list, a grocery list, or a shared checklist with friends.

### Why It Exists

Most list apps ask you to create an account, push everything to the cloud, and treat every checklist the same way.

- No registration required
- No mandatory cloud storage
- Local encrypted persistence in the browser
- Compressed data for lightweight sharing and syncing
- Async comparison and alignment with friends
- Useful for life targets, daily tasks, travel plans, shopping lists, and more

### Core Features

- **Private by default**: Your list data stays in local browser storage instead of a hosted account system.
- **No signup friction**: Open the app and start writing immediately.
- **Local encrypted + compressed storage**: State is encrypted in the browser and compressed for compact sharing.
- **Async sync flow**: Share a list or checklist with a friend and compare progress without needing a real-time backend.
- **Simple sharing**: Supports share links and QR-based handoff for lightweight collaboration.
- **Flexible list types**: Works well as a life list, bucket list, to-do list, checklist, or grocery list.

### Positioning

- A minimal web app for life goals and personal targets
- A shareable checklist without account creation
- A lightweight alternative to cloud-first to-do apps
- A privacy-friendly way to keep and compare lists

### Getting Started

1. Install dependencies:

```bash
npm install
```

2. Start the development server:

```bash
npm run dev
```

3. Build for production:

```bash
npm run build
```

### Tech Stack

- React
- Vite
- TypeScript
- Tailwind CSS
- Zustand
- LZ-String
- Web Crypto API
- Lucide React
- Motion
