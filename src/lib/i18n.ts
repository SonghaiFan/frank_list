...
const translations: Record<Locale, Record<string, TranslationValue>> = {
  zh: {
    ...
    "app.reset.body":
      "这会清空你本地所有组、勾选、自己新增的项目、装订记录，以及已导入的外部组，并恢复成默认第一组。",
  ...
  en: {
    ...
    "workspace.compareNotice":
      "A sync request arrived for this group. Please review each page below and then compare the results.",
    ...
    "page.addItemPlaceholder": translationFactory(
      'Add a new item to "{title}"...',
    ),
    ...
    "app.reset.body":
      "This clears all local groups, checkmarks, custom items, bound pages, and imported external groups, then restores the default first group.",
    "app.reset.confirm": "Reset",
    "app.delete.title": translationFactory('Delete "{title}"?'),
    "app.delete.body":
      "This cannot be undone, including custom items and checkmarks inside the group.",
