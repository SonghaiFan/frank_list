...
interface AppDataState {
  ...
  toggleTick: (params: { activeGroupId: string; itemId: string }) => ToggleTickResult;
}
...
export const useAppDataStore = create<AppDataState>((set, get) => ({
  ...
  addItem: ({ activeGroupId, text }) => {
    ...
    if (!activeGroup || activeGroup.items.some((item) => item.text === trimmed)) return;
    ...
    set({
      groups: groups.map((group) =>
        group.id === activeGroupId ? { ...group, items: [...group.items, newItem] } : group
      ),
      extraPageCounts: (() => {
  ...
  deleteGroup: (activeGroupId) => {
    ...
    const nextBoundPages = Object.fromEntries(
      Object.entries(boundPages).filter(([key]) => !key.startsWith(`${activeGroupId}:`))
    );
  ...
  pruneBoundPages: () => set((state) => {
    ...
    Object.keys(state.boundPages).forEach((pageKey) => {
      ...
      const isComplete = items.length === PAGE_SIZE && items.every((item) => !!state.myTicks[item.id]);
  ...
  removeItem: ({ activeGroupId, itemId }) => set((state) => {
  ...
  toggleTick: ({ activeGroupId, itemId }) => {
    ...
    if (isChecking) {
      const activeGroup = groups.find((group) => group.id === activeGroupId);
      const itemIndex = activeGroup?.items.findIndex((item) => item.id === itemId) ?? -1;
      if (activeGroup && itemIndex !== -1) {
        ...
        const isPageComplete = pageItems.length === PAGE_SIZE && pageItems.every((item) => !!nextTicks[item.id]);
